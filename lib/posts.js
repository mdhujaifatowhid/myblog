...
export async function createPost({ title, date, excerpt, thumbnail, content }) {
...
export async function updatePost(slug, { title, date, excerpt, thumbnail, content, sha }) {
  const frontmatter = matter.stringify(content || '', {
    title,
    date: date || new Date().toISOString().slice(0, 10),
    excerpt: excerpt || '',
